...
public class SwipeBackLayout extends FrameLayout {
    ...
    public void scrollToFinishActivity() {
        ...
        // mContentView:在setContentView中设置
        final int childWidth = mContentView.getWidth();
        final int childHeight = mContentView.getHeight();

        int left = 0, top = 0;

        // mEdgeFlag:在setEdgeTrackingEnabled中设置
        // mShadowLeft:在setShadow中设置
    ...
    public void attachToActivity(Activity activity) {
        mActivity = activity;
        /*
            TypedArray 是一个数组容器，在这个容器中装由
             obtainStyledAttributes(AttributeSet, int[], int, int)
             或者 obtainAttributes(AttributeSet, int[]) 函数获取到的属性值。
             用完之后记得调用 recycle() 函数回收资源。
             索引值用来获取 Attributes 对应的属性值（这个 Attributes 将会被传入
             obtainStyledAttributes() 函数）。
        */
