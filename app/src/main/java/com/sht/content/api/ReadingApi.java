...
public class ReadingApi {
    ...
    public static String getBookInfo(int position, BookBean bean){
        switch (position){
            case 0:
                if (Utils.hasString(bean.getSummary()) == false)
                    break;
                return bean.getSummary();
            case 1:
                if (Utils.hasString(bean.getCatalog()) == false)
                    break;
                return bean.getCatalog();
            case 2:
                if (Utils.hasString(bean.getAuthor_intro()) == false)
                    break;
                return bean.getAuthor_intro();
        }
        return "Soory,No Message Here";
    }
